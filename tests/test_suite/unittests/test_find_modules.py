...
from cosmic_ray.utils.modules import find_modules, filter_paths


def test_small_directory_tree(data_dir):
    paths = (('a', '__init__.py'), ('a', 'b.py'), ('a', 'py.py'),
             ('a', 'c', '__init__.py'), ('a', 'c', 'd.py'))
    expected = set(os.path.join(data_dir, *path) for path in paths)
    results = set(find_modules(os.path.join(data_dir, 'a')))
...
def test_finding_modules_via_dir_name(data_dir):
    ...
    expected = set(os.path.join(data_dir, *path) for path in paths)
    results = set(find_modules(os.path.join(data_dir, 'a', 'c')))
    assert expected == results
...
def test_finding_modules_via_dir_name_and_filename_ending_in_py(data_dir):
    paths = (('a', 'c', 'd.py'), )
    expected = set(os.path.join(data_dir, *path) for path in paths)
    results = set(find_modules(os.path.join(data_dir, 'a', 'c', 'd.py')))
    assert expected == results
...
def test_finding_module_py_dot_py_using_dots(data_dir):
    paths = (('a', 'py.py'), )
    expected = set(os.path.join(data_dir, *path) for path in paths)
    results = set(find_modules(os.path.join(data_dir, 'a', 'py.py')))
    assert expected == results


def test_finding_modules_py_dot_py_using_slashes(data_dir):
    expected = set()
    results = set(find_modules(data_dir / 'a' / 'py'))
    assert expected == results
...
def test_small_directory_tree_with_excluding_files(data_dir, path_utils):
    ...
    expected = set(os.path.join(*path) for path in paths)
